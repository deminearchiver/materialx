import { createThemeContract } from "@vanilla-extract/css";
import type { CSSVarFunction } from "~/utils";

export type BaselineTokens<T> = {
  primary0: T;
  primary10: T;
  primary20: T;
  primary30: T;
  primary40: T;
  primary50: T;
  primary60: T;
  primary70: T;
  primary80: T;
  primary90: T;
  primary95: T;
  primary98: T;
  primary99: T;
  primary100: T;
  secondary0: T;
  secondary10: T;
  secondary20: T;
  secondary30: T;
  secondary40: T;
  secondary50: T;
  secondary60: T;
  secondary70: T;
  secondary80: T;
  secondary90: T;
  secondary95: T;
  secondary98: T;
  secondary99: T;
  secondary100: T;
  tertiary0: T;
  tertiary10: T;
  tertiary20: T;
  tertiary30: T;
  tertiary40: T;
  tertiary50: T;
  tertiary60: T;
  tertiary70: T;
  tertiary80: T;
  tertiary90: T;
  tertiary95: T;
  tertiary98: T;
  tertiary99: T;
  tertiary100: T;
  neutral0: T;
  neutral4: T;
  neutral6: T;
  neutral10: T;
  neutral12: T;
  neutral17: T;
  neutral20: T;
  neutral22: T;
  neutral24: T;
  neutral30: T;
  neutral40: T;
  neutral50: T;
  neutral60: T;
  neutral70: T;
  neutral80: T;
  neutral87: T;
  neutral90: T;
  neutral92: T;
  neutral94: T;
  neutral95: T;
  neutral96: T;
  neutral98: T;
  neutral99: T;
  neutral100: T;
  neutralVariant0: T;
  neutralVariant10: T;
  neutralVariant20: T;
  neutralVariant30: T;
  neutralVariant40: T;
  neutralVariant50: T;
  neutralVariant60: T;
  neutralVariant70: T;
  neutralVariant80: T;
  neutralVariant90: T;
  neutralVariant95: T;
  neutralVariant98: T;
  neutralVariant99: T;
  neutralVariant100: T;
  error0: T;
  error10: T;
  error20: T;
  error30: T;
  error40: T;
  error50: T;
  error60: T;
  error70: T;
  error80: T;
  error90: T;
  error95: T;
  error98: T;
  error99: T;
  error100: T;
};

export type StaticTokens<T> = {
  white: T;
  black: T;
  blue0: T;
  blue10: T;
  blue20: T;
  blue30: T;
  blue40: T;
  blue50: T;
  blue60: T;
  blue70: T;
  blue80: T;
  blue90: T;
  blue95: T;
  blue98: T;
  blue100: T;
  blueVariant0: T;
  blueVariant10: T;
  blueVariant20: T;
  blueVariant30: T;
  blueVariant40: T;
  blueVariant50: T;
  blueVariant60: T;
  blueVariant70: T;
  blueVariant80: T;
  blueVariant90: T;
  blueVariant95: T;
  blueVariant98: T;
  blueVariant100: T;
  yellow0: T;
  yellow10: T;
  yellow20: T;
  yellow30: T;
  yellow40: T;
  yellow50: T;
  yellow60: T;
  yellow70: T;
  yellow80: T;
  yellow90: T;
  yellow95: T;
  yellow98: T;
  yellow100: T;
  red0: T;
  red10: T;
  red20: T;
  red30: T;
  red40: T;
  red50: T;
  red60: T;
  red70: T;
  red80: T;
  red90: T;
  red95: T;
  red98: T;
  red100: T;
  purple0: T;
  purple10: T;
  purple20: T;
  purple30: T;
  purple40: T;
  purple50: T;
  purple60: T;
  purple70: T;
  purple80: T;
  purple90: T;
  purple95: T;
  purple98: T;
  purple100: T;
  cyan0: T;
  cyan10: T;
  cyan20: T;
  cyan30: T;
  cyan40: T;
  cyan50: T;
  cyan60: T;
  cyan70: T;
  cyan80: T;
  cyan90: T;
  cyan95: T;
  cyan98: T;
  cyan100: T;
  grey0: T;
  grey10: T;
  grey20: T;
  grey30: T;
  grey40: T;
  grey50: T;
  grey60: T;
  grey70: T;
  grey80: T;
  grey90: T;
  grey95: T;
  grey98: T;
  grey100: T;
  greyVariant0: T;
  greyVariant10: T;
  greyVariant20: T;
  greyVariant30: T;
  greyVariant40: T;
  greyVariant50: T;
  greyVariant60: T;
  greyVariant70: T;
  greyVariant80: T;
  greyVariant90: T;
  greyVariant95: T;
  greyVariant98: T;
  greyVariant100: T;
  green0: T;
  green10: T;
  green20: T;
  green30: T;
  green40: T;
  green50: T;
  green60: T;
  green70: T;
  green80: T;
  green90: T;
  green95: T;
  green98: T;
  green100: T;
  orange0: T;
  orange10: T;
  orange20: T;
  orange30: T;
  orange40: T;
  orange50: T;
  orange60: T;
  orange70: T;
  orange80: T;
  orange90: T;
  orange95: T;
  orange98: T;
  orange100: T;
  pink0: T;
  pink10: T;
  pink20: T;
  pink30: T;
  pink40: T;
  pink50: T;
  pink60: T;
  pink70: T;
  pink80: T;
  pink90: T;
  pink95: T;
  pink98: T;
  pink100: T;
};

export type Tokens<T> = BaselineTokens<T> & StaticTokens<T>;

const BASELINE_PALETTE: BaselineTokens<string> = {
  primary0: "#000000",
  primary10: "#21005D",
  primary20: "#381E72",
  primary30: "#4F378B",
  primary40: "#6750A4",
  primary50: "#7F67BE",
  primary60: "#9A82DB",
  primary70: "#B69DF8",
  primary80: "#D0BCFF",
  primary90: "#EADDFF",
  primary95: "#F6EDFF",
  primary98: "#FEF7FF",
  primary99: "#FFFBFE",
  primary100: "#FFFFFF",
  secondary0: "#000000",
  secondary10: "#1D192B",
  secondary20: "#332D41",
  secondary30: "#4A4458",
  secondary40: "#625B71",
  secondary50: "#7A7289",
  secondary60: "#958DA5",
  secondary70: "#B0A7C0",
  secondary80: "#CCC2DC",
  secondary90: "#E8DEF8",
  secondary95: "#F6EDFF",
  secondary98: "#FEF7FF",
  secondary99: "#FFFBFE",
  secondary100: "#FFFFFF",
  tertiary0: "#000000",
  tertiary10: "#31111D",
  tertiary20: "#492532",
  tertiary30: "#633B48",
  tertiary40: "#7D5260",
  tertiary50: "#986977",
  tertiary60: "#B58392",
  tertiary70: "#D29DAC",
  tertiary80: "#EFB8C8",
  tertiary90: "#FFD8E4",
  tertiary95: "#FFECF1",
  tertiary98: "#FFF8F8",
  tertiary99: "#FFFBFA",
  tertiary100: "#FFFFFF",
  neutral0: "#000000",
  neutral4: "#0F0D13",
  neutral6: "#141218",
  neutral10: "#1D1B20",
  neutral12: "#211F26",
  neutral17: "#2B2930",
  neutral20: "#322F35",
  neutral22: "#36343B",
  neutral24: "#3B383E",
  neutral30: "#48464C",
  neutral40: "#605D64",
  neutral50: "#79767D",
  neutral60: "#938F96",
  neutral70: "#AEA9B1",
  neutral80: "#CAC5CD",
  neutral87: "#DED8E1",
  neutral90: "#E6E0E9",
  neutral92: "#ECE6F0",
  neutral94: "#F3EDF7",
  neutral95: "#F5EFF7",
  neutral96: "#F7F2FA",
  neutral98: "#FEF7FF",
  neutral99: "#FFFBFF",
  neutral100: "#FFFFFF",
  neutralVariant0: "#000000",
  neutralVariant10: "#1D1A22",
  neutralVariant20: "#322F37",
  neutralVariant30: "#49454F",
  neutralVariant40: "#605D66",
  neutralVariant50: "#79747E",
  neutralVariant60: "#938F99",
  neutralVariant70: "#AEA9B4",
  neutralVariant80: "#CAC4D0",
  neutralVariant90: "#E7E0EC",
  neutralVariant95: "#F5EEFA",
  neutralVariant98: "#FDF7FF",
  neutralVariant99: "#FFFBFE",
  neutralVariant100: "#FFFFFF",
  error0: "#000000",
  error10: "#410E0B",
  error20: "#601410",
  error30: "#8C1D18",
  error40: "#B3261E",
  error50: "#DC362E",
  error60: "#E46962",
  error70: "#EC928E",
  error80: "#F2B8B5",
  error90: "#F9DEDC",
  error95: "#FCEEEE",
  error98: "#FFF8F7",
  error99: "#FFFBF9",
  error100: "#FFFFFF",
};

const STATIC_PALETTE: StaticTokens<string> = {
  white: "#FFFFFF",
  black: "#000000",
  blue0: "#000000",
  blue10: "#001944",
  blue20: "#012C6F",
  blue30: "#04409F",
  blue40: "#1157CE",
  blue50: "#3271EA",
  blue60: "#4E8FF8",
  blue70: "#76ACFF",
  blue80: "#A1C9FF",
  blue90: "#D0E4FF",
  blue95: "#E7F2FF",
  blue98: "#F5FAFF",
  blue100: "#FFFFFF",
  blueVariant0: "#000000",
  blueVariant10: "#001F2D",
  blueVariant20: "#003549",
  blueVariant30: "#004D68",
  blueVariant40: "#006788",
  blueVariant50: "#0081A8",
  blueVariant60: "#009DC9",
  blueVariant70: "#00BBEA",
  blueVariant80: "#67D4FF",
  blueVariant90: "#BDE9FF",
  blueVariant95: "#E0F4FF",
  blueVariant98: "#F4FAFF",
  blueVariant100: "#FFFFFF",
  yellow0: "#000000",
  yellow10: "#2F1400",
  yellow20: "#4D2600",
  yellow30: "#6D3A01",
  yellow40: "#8F4E06",
  yellow50: "#B16300",
  yellow60: "#D37B00",
  yellow70: "#EF9800",
  yellow80: "#FCBD00",
  yellow90: "#FFE07C",
  yellow95: "#FFE07C",
  yellow98: "#FFFADE",
  yellow100: "#FFFFFF",
  red0: "#000000",
  red10: "#3A0907",
  red20: "#60150F",
  red30: "#8A1A16",
  red40: "#B3251E",
  red50: "#DB372D",
  red60: "#F55E57",
  red70: "#FF8983",
  red80: "#FFB3AE",
  red90: "#FFDADC",
  red95: "#FFECEE",
  red98: "#FFF8F8",
  red100: "#FFFFFF",
  purple0: "#000000",
  purple10: "#280255",
  purple20: "#400B84",
  purple30: "#5629A4",
  purple40: "#7438D2",
  purple50: "#9254EA",
  purple60: "#AD72FF",
  purple70: "#C597FF",
  purple80: "#D9BAFD",
  purple90: "#EEDCFE",
  purple95: "#F7ECFE",
  purple98: "#FDF8FF",
  purple100: "#FFFFFF",
  cyan0: "#000000",
  cyan10: "#001F26",
  cyan20: "#003641",
  cyan30: "#004E5D",
  cyan40: "#00687C",
  cyan50: "#00839B",
  cyan60: "#009EBB",
  cyan70: "#00BBDF",
  cyan80: "#60D5F3",
  cyan90: "#ACEDFF",
  cyan95: "#D8F6FF",
  cyan98: "#F0FBFF",
  cyan100: "#FFFFFF",
  grey0: "#000000",
  grey10: "#1B1B1C",
  grey20: "#303030",
  grey30: "#474747",
  grey40: "#5E5E5E",
  grey50: "#777777",
  grey60: "#919191",
  grey70: "#ABABAB",
  grey80: "#C7C7C7",
  grey90: "#E3E3E3",
  grey95: "#F2F2F2",
  grey98: "#F9F9F9",
  grey100: "#FFFFFF",
  greyVariant0: "#000000",
  greyVariant10: "#191D1C",
  greyVariant20: "#2D312F",
  greyVariant30: "#444746",
  greyVariant40: "#5C5F5E",
  greyVariant50: "#747775",
  greyVariant60: "#8E918F",
  greyVariant70: "#A9ACAA",
  greyVariant80: "#C4C7C5",
  greyVariant90: "#E1E3E1",
  greyVariant95: "#EFF2EF",
  greyVariant98: "#F7FAF7",
  greyVariant100: "#FFFFFF",
  green0: "#000000",
  green10: "#002110",
  green20: "#00381F",
  green30: "#00522C",
  green40: "#006C35",
  green50: "#128937",
  green60: "#1AA64A",
  green70: "#44C265",
  green80: "#80DA88",
  green90: "#BEEFBB",
  green95: "#DDF8D8",
  green98: "#F2FCEF",
  green100: "#FFFFFF",
  orange0: "#000000",
  orange10: "#321200",
  orange20: "#522302",
  orange30: "#753403",
  orange40: "#9A4600",
  orange50: "#C05A01",
  orange60: "#E86E00",
  orange70: "#FF8D41",
  orange80: "#FFB683",
  orange90: "#FFDCC3",
  orange95: "#FFEDE1",
  orange98: "#FFF8F4",
  orange100: "#FFFFFF",
  pink0: "#000000",
  pink10: "#3D0023",
  pink20: "#620438",
  pink30: "#8D0053",
  pink40: "#B60D6E",
  pink50: "#DC258D",
  pink60: "#F94AAB",
  pink70: "#FF7DD2",
  pink80: "#FFAEE4",
  pink90: "#FFD8EF",
  pink95: "#FFECF6",
  pink98: "#FFF7FC",
  pink100: "#FFFFFF",
};

const PALETTE: Tokens<string> = {
  ...BASELINE_PALETTE,
  ...STATIC_PALETTE,
};

export const createBaselineTokens = (): BaselineTokens<CSSVarFunction> => {
  const template: BaselineTokens<null> = {
    primary0: null,
    primary10: null,
    primary20: null,
    primary30: null,
    primary40: null,
    primary50: null,
    primary60: null,
    primary70: null,
    primary80: null,
    primary90: null,
    primary95: null,
    primary98: null,
    primary99: null,
    primary100: null,
    secondary0: null,
    secondary10: null,
    secondary20: null,
    secondary30: null,
    secondary40: null,
    secondary50: null,
    secondary60: null,
    secondary70: null,
    secondary80: null,
    secondary90: null,
    secondary95: null,
    secondary98: null,
    secondary99: null,
    secondary100: null,
    tertiary0: null,
    tertiary10: null,
    tertiary20: null,
    tertiary30: null,
    tertiary40: null,
    tertiary50: null,
    tertiary60: null,
    tertiary70: null,
    tertiary80: null,
    tertiary90: null,
    tertiary95: null,
    tertiary98: null,
    tertiary99: null,
    tertiary100: null,
    neutral0: null,
    neutral4: null,
    neutral6: null,
    neutral10: null,
    neutral12: null,
    neutral17: null,
    neutral20: null,
    neutral22: null,
    neutral24: null,
    neutral30: null,
    neutral40: null,
    neutral50: null,
    neutral60: null,
    neutral70: null,
    neutral80: null,
    neutral87: null,
    neutral90: null,
    neutral92: null,
    neutral94: null,
    neutral95: null,
    neutral96: null,
    neutral98: null,
    neutral99: null,
    neutral100: null,
    neutralVariant0: null,
    neutralVariant10: null,
    neutralVariant20: null,
    neutralVariant30: null,
    neutralVariant40: null,
    neutralVariant50: null,
    neutralVariant60: null,
    neutralVariant70: null,
    neutralVariant80: null,
    neutralVariant90: null,
    neutralVariant95: null,
    neutralVariant98: null,
    neutralVariant99: null,
    neutralVariant100: null,
    error0: null,
    error10: null,
    error20: null,
    error30: null,
    error40: null,
    error50: null,
    error60: null,
    error70: null,
    error80: null,
    error90: null,
    error95: null,
    error98: null,
    error99: null,
    error100: null,
  };
  return createThemeContract(template);
};

export const createStaticTokens = (): StaticTokens<CSSVarFunction> => {
  const template: StaticTokens<null> = {
    white: null,
    black: null,
    blue0: null,
    blue10: null,
    blue20: null,
    blue30: null,
    blue40: null,
    blue50: null,
    blue60: null,
    blue70: null,
    blue80: null,
    blue90: null,
    blue95: null,
    blue98: null,
    blue100: null,
    blueVariant0: null,
    blueVariant10: null,
    blueVariant20: null,
    blueVariant30: null,
    blueVariant40: null,
    blueVariant50: null,
    blueVariant60: null,
    blueVariant70: null,
    blueVariant80: null,
    blueVariant90: null,
    blueVariant95: null,
    blueVariant98: null,
    blueVariant100: null,
    yellow0: null,
    yellow10: null,
    yellow20: null,
    yellow30: null,
    yellow40: null,
    yellow50: null,
    yellow60: null,
    yellow70: null,
    yellow80: null,
    yellow90: null,
    yellow95: null,
    yellow98: null,
    yellow100: null,
    red0: null,
    red10: null,
    red20: null,
    red30: null,
    red40: null,
    red50: null,
    red60: null,
    red70: null,
    red80: null,
    red90: null,
    red95: null,
    red98: null,
    red100: null,
    purple0: null,
    purple10: null,
    purple20: null,
    purple30: null,
    purple40: null,
    purple50: null,
    purple60: null,
    purple70: null,
    purple80: null,
    purple90: null,
    purple95: null,
    purple98: null,
    purple100: null,
    cyan0: null,
    cyan10: null,
    cyan20: null,
    cyan30: null,
    cyan40: null,
    cyan50: null,
    cyan60: null,
    cyan70: null,
    cyan80: null,
    cyan90: null,
    cyan95: null,
    cyan98: null,
    cyan100: null,
    grey0: null,
    grey10: null,
    grey20: null,
    grey30: null,
    grey40: null,
    grey50: null,
    grey60: null,
    grey70: null,
    grey80: null,
    grey90: null,
    grey95: null,
    grey98: null,
    grey100: null,
    greyVariant0: null,
    greyVariant10: null,
    greyVariant20: null,
    greyVariant30: null,
    greyVariant40: null,
    greyVariant50: null,
    greyVariant60: null,
    greyVariant70: null,
    greyVariant80: null,
    greyVariant90: null,
    greyVariant95: null,
    greyVariant98: null,
    greyVariant100: null,
    green0: null,
    green10: null,
    green20: null,
    green30: null,
    green40: null,
    green50: null,
    green60: null,
    green70: null,
    green80: null,
    green90: null,
    green95: null,
    green98: null,
    green100: null,
    orange0: null,
    orange10: null,
    orange20: null,
    orange30: null,
    orange40: null,
    orange50: null,
    orange60: null,
    orange70: null,
    orange80: null,
    orange90: null,
    orange95: null,
    orange98: null,
    orange100: null,
    pink0: null,
    pink10: null,
    pink20: null,
    pink30: null,
    pink40: null,
    pink50: null,
    pink60: null,
    pink70: null,
    pink80: null,
    pink90: null,
    pink95: null,
    pink98: null,
    pink100: null,
  };
  return createThemeContract(template);
};

export const createTokens = (): Tokens<CSSVarFunction> => {
  return {
    ...createBaselineTokens(),
    ...createStaticTokens(),
  };
};

export const createBaselineValues = (): BaselineTokens<string> =>
  BASELINE_PALETTE;

export const createStaticValues = (): StaticTokens<string> => STATIC_PALETTE;

export const createValues = (): Tokens<string> => PALETTE;
